...
public class PhoneBook implements ShellDependent {
    ...
    @Command
   public Collection<Record> returnUntil (String name){
       return indexByName.headMap(name).values();

    }

    @Command
    public Collection<Record> returnFromTo(String from,String to){
        return indexByName.subMap(from,false,to,true).values();

    }

    @Command
    public Collection<Record> findAfter(String from){
        return indexByName.tailMap(from,false).values();

    }


    @Command
    public Collection<Record> sortDesc(){
        return indexByName.descendingMap().values();
